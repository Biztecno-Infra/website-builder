...
import { getDefaultBlockProperties, globalStyle } from "@utils/constant";
import { Block, BlockType, IGridCellProps, IBlockContext, IBlocksState, GlobalStyles } from "../types";
...
export const useBlocks = () : IBlockContext => {
      ...
      if (block.type === BlockType.GRID && property === 'columns') {
        ...
        if (columnDiff > 0) {
          ...
          const newGridCells = Object.fromEntries(
            newGridCellIds.map(id => [id, { $set: { id, type: BlockType.GRIDCELL, parentId: blockId, childBlocks: [] } }])
          );
          ...
          const removeUpdates = Object.fromEntries(removedGridCells.map((id:any) => [id, { $unset: [id] }]));
  ...
  const onDeleteBlock = (blockId: string) => {
    ...
    if(!deleteBlock.parentId) {
      setRootBlockOrder((prevOrder) => prevOrder.filter((id) => id !== blockId));
    }
  };
  ...
  const handleSwappingV2 = (dragSrc: any, dropAreaId: string) => {
    setBlocks(prvsBlockState => {
      ...
      const insertOrDeleteBlock = (blockId: string, { deleteIndex, insertIndex, insertValue }: { deleteIndex?: number, insertIndex?: number, insertValue?: string }) => {
      ...
      const dropBlockParent = dropBlock?.parentId ? prvsBlockState[dropBlock.parentId] : undefined;
      const dragBlock = prvsBlockState[dragSrc.id];
      const dragBlockParent = dragBlock.parentId ? prvsBlockState[dragBlock.parentId] : undefined;
      const dropNodeIndex = dropBlockParent ? dropBlockParent.childBlocks?.findIndex(id => id === dropBlock?.id) : 0;
      const dragNodeIndex = dragBlockParent ? dragBlockParent.childBlocks?.findIndex(id => id === dragBlock?.id) : 0;
      ...
      if (dragBlock?.parentId) {
        ...
        if (dropBlock?.parentId) {
          ...
          if (dragBlock.parentId === dropBlock.parentId) {
            ...
            insertOrDeleteBlock(dropBlock.parentId, { deleteIndex: dragNodeIndex, insertIndex: dropNodeIndex, insertValue: dropBlockParent?.childBlocks[dragNodeIndex] || "" })
          } else {
            // drag to other grid or grid cell

            if (
              (dropBlockParent?.type === BlockType.GRIDCELL && dragBlockParent?.type === BlockType.GRIDCELL) ||
              (dragBlock.type === BlockType.GRIDCELL && dropBlock?.type === BlockType.GRIDCELL)
            ) {
              // we are shfiting the elements of grid cell from one to another
              insertOrDeleteBlock(dropBlock.parentId, { insertIndex: dropNodeIndex, insertValue: dragBlock.id });

              insertOrDeleteBlock(dragBlock.parentId, { deleteIndex: dragNodeIndex });
              updateParentOfBlock(dragBlock.id, dropBlock.parentId);

              if(prvsBlockState[dragBlock.parentId]?.type === BlockType.GRID && prvsBlockState[dragBlock.parentId].childBlocks?.length === 0) {
              ...
              prvsBlockState = update(prvsBlockState, {
                [dropBlock.parentId]: {
                  columns: { $set: prvsBlockState [dropBlock.parentId].childBlocks.length},
                  cellWidths: { $set: getDistributtedLength(prvsBlockState[dropBlock.parentId].childBlocks.length)}
                },
                [dragBlock.parentId]: {
                  columns: { $set: prvsBlockState[dragBlock.parentId].childBlocks.length},
                  cellWidths: { $set: getDistributtedLength(prvsBlockState[dragBlock.parentId].childBlocks.length)}
                }
              });

            } else if (dragBlock.type !== BlockType.GRIDCELL && dropBlock?.type === BlockType.GRIDCELL) {

              // when we select any text and send that drop over the grid which is already a part of grid cell

              insertOrDeleteBlock(dropBlock.id, { insertIndex: dropBlock.childBlocks.length, insertValue: dragBlock.id });
          ...
          setRootBlockOrder(prevs => {
            const dropNodeIndex = dropBlock?.id ? prevs.findIndex(id => id === dropBlock?.id) : prevs.length;
            return update(prevs, {
              $splice: [
                [dropNodeIndex, 0, newGridBlock?.id || dragBlock.id]
              ]
            })
          });
        ...
        if (dropBlock?.parentId) {
          ...
          insertOrDeleteBlock(isGridCell ? dropBlock.id : dropBlock.parentId, { insertIndex: isGridCell ? dropBlock.childBlocks.length : dropNodeIndex, insertValue: dragBlock.id });
          updateParentOfBlock(dragBlock.id, isGridCell ? dropBlock.id : dropBlock.parentId);
  ...
  const handleInsertion = (dragSrc: any, dropAreaId: string) => {
    ...
    const { defaultBlock, extraBlocks } = initializeBlock(blockProprtys as Block);
      ...
      if (dropBlock) {
        if (dropBlock?.type === BlockType.GRIDCELL) {
          ...
          if(dropBlockParent?.type === BlockType.GRIDCELL) {
            const dropNodeIndex = (dropBlockParent as IGridCellProps).childBlocks.findIndex(id => dropBlock.id === id);
            prevsBlocks = update(blocks, {
              [dropBlockParent.id]: { childBlocks: { $splice: [[dropNodeIndex, 0, blockID]] } },
              [blockID]: { $set: { ...defaultBlock, parentId: dropBlockParent.id } }
            })
          } else {
          ...
          const indexOfDropArea = rootBlockOrder.findIndex(value => value === dropBlock.id);

          setRootBlockOrder(prevs => update(prevs, {
   ...
   */
  const handleDropper = useCallback((dragSrc: any, dropAreaId: string) => {
    if(dragSrc.id) {
   ...
   const blocksToJson = (
    ...
    const layout = {
      root: {
        type: "EmailLayout",
        data: {
          backdropColor: globalStyles?.backdropColor,
 ...
 function convertJsonToHtml(jsonData: any, rootStyles: GlobalStyles) {
  ...
  const rawHtml = `
    <!DOCTYPE html>
      ...
      <body>
        <table style="font-family:${rootStyles?.fontFamily}; width:600px; max-width:600px; margin:0 auto; background-color:${rootStyles?.backdropColor}; color:${rootStyles?.textColor}; ${tableCommonStyle}">
          <tbody>
            <tr>
