...
.acc-item{display:flex;flex-direction:column;width:100%;gap:var(--acc-content-gap,0px)}
...
function renderElement(el: CanvasElement): string {
  ...
  if (el.type === 'form') {
    return renderForm(el, `${cStyle};padding:${pad}`, `el-${el.id}`, '');
  }
  const inner = renderElementInner(el, pad, cStyle);
  const link = resolveElementHref(el);
  return link
    ? `<a href="${link.href}" target="${link.target}"${link.onclick ? ` onclick="${link.onclick}"` : ''} style="display:block;text-decoration:none;color:inherit" class="el-${el.id}">${inner}</a>`
    : `<div class="el-${el.id}">${inner}</div>`;
}
...
function renderGridElement(el: CanvasElement): string {
  ...
  const wrapStyle = `${heightCss}box-sizing:border-box;opacity:${el.style.opacity}${shadowCss}${rotateCss}${wrapRadiusCss}`;
  ...
  if (el.type === 'form') {
    ...
    return renderForm(el, `${bgb ? bgb + ';' : ''}${wrapStyle};padding:${pad}`, `ge-${el.id}`, '');
  }
  ...
  const link = resolveElementHref(el);
  if (link) {
    return `<a href="${link.href}" target="${link.target}"${link.onclick ? ` onclick="${link.onclick}"` : ''} style="${wrapStyle};display:block;text-decoration:none;color:inherit" class="ge-${el.id}">${inner}</a>`;
  }
...
function renderGridSection(sec: GridSection, nodes: NodeMap, pageFixed: boolean, pageMaxWidth: number): string {
  ...
  const secMargin = sec.style.margin;
  const marginCss = secMargin
    ? `;margin:${secMargin.top ?? 0}px ${secMargin.right ?? 0}px ${secMargin.bottom ?? 0}px ${secMargin.left ?? 0}px`
    : '';

  const secShadowG = sec.style.shadow;
  const shadowCssG = secShadowG?.enabled
    ? `;box-shadow:${secShadowG.x}px ${secShadowG.y}px ${secShadowG.blur}px ${secShadowG.spread}px ${secShadowG.color}`
    : '';
...
function renderFlexSection(sec: FlexSection, nodes: NodeMap, pageFixed: boolean, pageMaxWidth: number): string {
  ...
  const secMarginF = sec.style.margin;
  const marginCssF = secMarginF
    ? `;margin:${secMarginF.top ?? 0}px ${secMarginF.right ?? 0}px ${secMarginF.bottom ?? 0}px ${secMarginF.left ?? 0}px`
    : '';

  const secShadowF = sec.style.shadow;
  const shadowCssF = secShadowF?.enabled
    ? `;box-shadow:${secShadowF.x}px ${secShadowF.y}px ${secShadowF.blur}px ${secShadowF.spread}px ${secShadowF.color}`
    : '';
...
function renderAccordion(accordion: Accordion, nodes: NodeMap): string {
  ...
  const containerStyle = containerBorder
    ...
    : '';
  return `<div class="acc acc-${accordion.id}" data-accordion data-acc-multiple="${p.allowMultiple ? 1 : 0}" style="--acc-icon-rot:${iconRot}deg;--acc-content-gap:${p.contentGap ?? 0}px;gap:${effItemGap}px${containerStyle}">${itemsHtml}</div>`;
}
...
function renderSection(sec: Section, nodes: NodeMap, pageFixed: boolean, pageMaxWidth: number): string {
  ...
  const freeMargin = sec.style.margin;
  const freeMarginCss = freeMargin
    ? `;margin:${freeMargin.top ?? 0}px ${freeMargin.right ?? 0}px ${freeMargin.bottom ?? 0}px ${freeMargin.left ?? 0}px`
    : '';
  const freeShadow = sec.style.shadow;
...
function generateCellCSS(
  ...
  tabletRules: string[],
  mobileRules: string[],
  sectionMode: 'grid' | 'flex' = 'grid',
): void {
  const desktopMode = cell.style.layoutMode ?? 'column';

  const baseSizeCss = sectionMode === 'flex'
    ? `flex:0 0 ${Math.round((Math.min(cell.columnSpan, 12) / 12) * 100)}%;min-width:0`
    : `grid-column:span ${Math.min(cell.columnSpan, 12)}${(cell.rowSpan ?? 1) > 1 ? `;grid-row:span ${cell.rowSpan}` : ''}`;

  baseRules.push(`.gc-${cell.id}{${baseSizeCss};display:flex;${cellDirectionCss(desktopMode)};align-items:${cell.style.alignItems};justify-content:${cell.style.justifyContent}}`);
  ...
  if (tCell?.hidden) {
    ...
    const tParts: string[] = [];
    if (tCell?.columnSpan !== undefined) {
      tParts.push(sectionMode === 'flex'
        ? `flex:0 0 ${Math.round((Math.min(tCell.columnSpan, 12) / 12) * 100)}%`
        : `grid-column:span ${Math.min(tCell.columnSpan, 12)}`);
    }
  ...
  if (mCell?.hidden) {
    ...
    const mParts: string[] = [];
    if (mCell?.columnSpan !== undefined) {
      mParts.push(sectionMode === 'flex'
        ? `flex:0 0 ${Math.round((Math.min(mCell.columnSpan, 12) / 12) * 100)}%`
        : `grid-column:span ${Math.min(mCell.columnSpan, 12)}`);
    }
  ...
  for (const elId of cell.children) {
    ...
    const marginCss = m ? `;margin:${m.top ?? 0}px ${m.right ?? 0}px ${m.bottom ?? 0}px ${m.left ?? 0}px` : '';
    baseRules.push(`.ge-${el.id}{${flexItemClassCss(el.flexLayout, desktopMode)}${marginCss}}`);
    const typo = el.style.typography;
    const tyLs = typo.letterSpacing ? `;letter-spacing:${typo.letterSpacing}px` : '';
    const tyTt = (typo.textTransform && typo.textTransform !== 'none') ? `;text-transform:${typo.textTransform}` : '';
    baseRules.push(`.ec-${el.id}{font-family:${typo.family};font-size:${typo.size}px;font-weight:${typo.weight};color:${typo.color};text-align:${typo.align};line-height:${typo.lineHeight}${tyLs}${tyTt}}`);
...
function generateElementCSS(sections: Section[], nodes: NodeMap): string {
  ...
  for (const sec of sections) {
    ...
    if (sec.layoutMode === 'flex') {
      const fSec = sec as import('../types').FlexSection;
      const fcfg = fSec.grid;
      baseRules.push(`.sc-flex-${fSec.id}{gap:${fcfg.rowGap}px ${fcfg.gap}px}`);
      const ftGap    = fSec.responsive?.tablet?.gap    ?? fcfg.gap;
      const ftRowGap = fSec.responsive?.tablet?.rowGap ?? fcfg.rowGap;
      const fmGap    = fSec.responsive?.mobile?.gap    ?? ftGap;
      const fmRowGap = fSec.responsive?.mobile?.rowGap ?? ftRowGap;
      if (ftGap !== fcfg.gap || ftRowGap !== fcfg.rowGap)
        tabletRules.push(`.sc-flex-${fSec.id}{gap:${ftRowGap}px ${ftGap}px}`);
      if (fmGap !== ftGap || fmRowGap !== ftRowGap)
        mobileRules.push(`.sc-flex-${fSec.id}{gap:${fmRowGap}px ${fmGap}px}`);
      for (const cellId of sec.children) {
        const cell = nodes[cellId] as GridCell | undefined;
        if (cell) generateCellCSS(cell, nodes, baseRules, tabletRules, mobileRules, 'flex');
      }
      emitSectionSharedCSS(sec, baseRules, tabletRules, mobileRules);
      continue;
    }
